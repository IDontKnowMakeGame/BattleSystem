...
public class Spear : Weapon
{
	...
	private void ReadyAnimation(Vector3 vec)
    {
		...
		{
			InGame.Player.SpriteTransform.localScale = vec == Vector3.left ? new Vector3(Math.Abs(InGame.Player.SpriteTransform.localScale.x), 1, 1) 
				: new Vector3(-(Math.Abs(InGame.Player.SpriteTransform.localScale.x)), 1, 1);
